...
import 'package:spdms_app/features/team/widgets/student_search_dialog.dart';
...
class _TeamDetailsPageState extends State<TeamDetailsPage> {
  ...
  Future<void> _addMember(String regNo) async {
    try {
      final response = await getIt<TeamProxyService>().post(
        Uri.parse('${ApiConfig.baseUrl}/api/v1/teams/${widget.teamId}/add-member?regNo=$regNo'),
        headers: {'Authorization': 'Bearer ${context.read<AuthProvider>().token!}'},
      );
      final data = jsonDecode(response.body);
      if (!mounted) return;
  ...
  Future<void> _removeMember(String regNo) async {
    try {
      final response = await getIt<TeamProxyService>().post(
        Uri.parse('${ApiConfig.baseUrl}/api/v1/teams/${widget.teamId}/remove-member?regNo=$regNo'),
        headers: {'Authorization': 'Bearer ${context.read<AuthProvider>().token!}'},
      );
      final data = jsonDecode(response.body);
      if (!mounted) return;
  ...
  Future<void> _changeCaptain(String regNo) async {
    try {
      final response = await getIt<TeamProxyService>().post(
        Uri.parse('${ApiConfig.baseUrl}/api/v1/teams/${widget.teamId}/captain?regNo=$regNo'),
        headers: {'Authorization': 'Bearer ${context.read<AuthProvider>().token!}'},
      );
      final data = jsonDecode(response.body);
      if (!mounted) return;
  ...
  void _showDeleteTeamDialog() {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete the team "${_team!.name}"? This action cannot be undone.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
          ElevatedButton(
            onPressed: () {
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return Scaffold(
        appBar: AppBar(title: const Text('Team Details'), backgroundColor: Colors.indigo, foregroundColor: Colors.white),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: _fetchTeamDetails),
        ],
      ),
        ...
        child: Column(
          ...
          children: [
            ...
            if (widget.canManage) ...[
              ...
              const Text('Management Actions', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Color(0xFF0F172A))),
              const SizedBox(height: 12),
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: [
                  ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(backgroundColor: Colors.green.shade50, foregroundColor: Colors.green.shade700),
                  ),
                  ElevatedButton.icon(
            ...
            Row(
              ...
              children: [
                Text('Team Members (${members.length}/${_team!.size})', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Color(0xFF0F172A))),
              ],
            ),
            const SizedBox(height: 12),
            if (members.isEmpty)
              const Center(child: Padding(padding: EdgeInsets.all(24.0), child: Text('No members in this team', style: TextStyle(color: Colors.grey))))
            else
              ...members.map((m) => TeamMemberCard(
                member: m,
        ...
        child: Column(
          ...
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Expanded(child: Text(_team!.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 22, color: Colors.indigo))),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                  decoration: BoxDecoration(color: Colors.amber.shade100, borderRadius: BorderRadius.circular(12)),
                  child: Text('Level $stage', style: TextStyle(color: Colors.amber.shade800, fontWeight: FontWeight.bold, fontSize: 12)),
                ),
              ],
            ),
            const SizedBox(height: 16),
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Expanded(child: _buildInfoItem('Department', getVal(_team!.departmentName))),
                Expanded(child: _buildInfoItem('Academic Year', getVal(_team!.academicYear))),
              ],
            ),
            const SizedBox(height: 12),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Expanded(child: _buildInfoItem('Year', getVal(_team!.yearName))),
                Expanded(child: _buildInfoItem('Semester', getVal(_team!.semesterName))),
              ],
            ),
            const SizedBox(height: 12),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Expanded(child: _buildInfoItem('Section', getVal(_team!.sectionName))),

...
import 'package:pragatix/features/teacher/pages/teacher_activity_workflow_page.dart';
...
class TeacherActivityListPage extends StatefulWidget {
  final int? stageId;
  ...
  @override
  State<TeacherActivityListPage> createState() =>
      _TeacherActivityListPageState();
}

class _TeacherActivityListPageState extends State<TeacherActivityListPage> {
  ...
  Future<void> _fetchActivities() async {
    ...
    try {
      final list = await _activityService.fetchActivities(
        stageId: widget.stageId ?? 1,
    ...
    setState(() {
      _filteredActivities = _activities.where((act) {
        final matchesQuery = query.isEmpty ||
            ...
            (act.subgroup != null &&
                act.subgroup!.toLowerCase().contains(query));
  ...
  void _onActivityTapped(ActivityModel activity) {
    final type = activity.type.toLowerCase();
    if (type.contains('group')) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => GroupActivityYearPage(
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => TeacherActivityWorkflowPage(
            activity: activity,
            stageId: widget.stageId,
  ...
  Widget build(BuildContext context) {
    final title = widget.categoryTitle != null
        ...
        : (widget.stageName != null ? widget.stageName! : 'Activities');

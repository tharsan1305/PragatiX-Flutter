...
class BadgeClaimsTab extends StatefulWidget {
  final String token;
  const BadgeClaimsTab({super.key, required this.token});

  @override
  State<BadgeClaimsTab> createState() => _BadgeClaimsTabState();
}

class _BadgeClaimsTabState extends State<BadgeClaimsTab> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<BadgeProvider>(context, listen: false).fetchTeacherPendingClaims(widget.token);
    });
  }
      ...
      body: claims.isEmpty
          ? Center(
              child: RefreshIndicator(
                onRefresh: () async {
                  await Provider.of<BadgeProvider>(context, listen: false).fetchTeacherPendingClaims(widget.token);
                },
          ...
          : RefreshIndicator(
              ...
              child: ListView.builder(
                ...
                final studentId = claim["studentId"] ?? "ID N/A";
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            TextButton(
                              onPressed: () {
                                badgeProvider.rejectClaim(widget.token, id).then((response) {
                                  if (context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                            ...
                            ElevatedButton(
                              onPressed: () {
                                badgeProvider.approveClaim(widget.token, id).then((response) {
                                  if (context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(

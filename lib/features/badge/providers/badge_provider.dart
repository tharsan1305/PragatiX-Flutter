...
class BadgeProvider extends ChangeNotifier {
  ...
  Future<Map<String, dynamic>> requestBadgeWorkflow(String token, int badgeId, String proofLink) async {
  ...
  Future<Map<String, dynamic>> approveBadgeWorkflow(String token, int requestId, String role) async {
    final response = await _repository.approveWorkflowRequest(token, requestId, role);
    if (response['success'] == true) {
  ...
  Future<Map<String, dynamic>> rejectBadgeWorkflow(String token, int requestId, String role, {String? remarks}) async {
    final response = await _repository.rejectWorkflowRequest(token, requestId, role, remarks: remarks);
    if (response['success'] == true) {
      await fetchAdminCCBadgeRequests(token, role);
    }
    return response;

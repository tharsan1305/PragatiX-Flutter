...
class CaptainGroupTab extends StatefulWidget {
  const CaptainGroupTab({super.key, });
...
class _CaptainGroupTabState extends State<CaptainGroupTab> {
  ...
  Future<void> _submitCreateGroup(String name, int size, String membersStr) async {
    final List<String> memberIds = membersStr.split(',')
        .map((s) => s.trim())
        .where((s) => s.isNotEmpty)
        .toList();

    try {
      final response = await getIt<CaptainProxyService>().post(
  ...
  Future<void> _addMember(String regNo) async {
    try {
      final response = await getIt<CaptainProxyService>().post(
        Uri.parse('${ApiConfig.baseUrl}/api/v1/teams/my-team/add-member?regNo=$regNo'),
        headers: {
          'Authorization': 'Bearer ${context.read<AuthProvider>().token!}',
        },
      );
  ...
  Future<void> _removeMember(String regNo, String name) async {
    try {
      final response = await getIt<CaptainProxyService>().post(
        Uri.parse('${ApiConfig.baseUrl}/api/v1/teams/my-team/remove-request?regNo=$regNo'),
        headers: {
          'Authorization': 'Bearer ${context.read<AuthProvider>().token!}',
        },
      );
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                _addMember(selectedStudent!['regNo']);
              },
              style: ElevatedButton.styleFrom(backgroundColor: Colors.amber),
              ...
              : _members.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isCaptain = member['regNo'] == captainId;
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text("Reg No: ${member["regNo"]}"),
                            trailing: isCaptain
                                ? const Chip(
                                ...
                                : IconButton(
                                    ...
                                    onPressed: () => _removeMember(member['regNo'], member['fullName']),
                                  ),
                          ),
                        );
                      },

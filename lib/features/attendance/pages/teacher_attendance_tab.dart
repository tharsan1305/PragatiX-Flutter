...
class _TeacherAttendanceTabState extends State<TeacherAttendanceTab> {
  ...
  Future<void> _loadLookups() async {
    try {
      ...
      setState(() {
        ...
        if (_academicYears.isNotEmpty) _academicYearId = _academicYears.first['id'];
        ...
        if (currentUser != null) {
           ...
           if (assignedDepartment != null && _departments.isNotEmpty) {
             final match = _departments.firstWhere((d) {
               ...
               return dName == assignedDepartment || dDeptName == assignedDepartment || dCode == assignedDepartment;
             }, orElse: () => null);
             if (match != null) _departmentId = match['id'];
           }
           if (assignedSectionId != null && _sections.isNotEmpty) {
             final match = _sections.firstWhere((s) => s['id'] == assignedSectionId, orElse: () => null);
        ...
        if (_departmentId == null && _departments.isNotEmpty) _departmentId = _departments.first['id'];
  ...
  Future<void> _saveAttendance() async {
    if (_students == null || _yearId == null || _departmentId == null || _academicYearId == null) return;
    
    setState(() => _isLoading = true);
    try {
      final dateStr = DateFormat('yyyy-MM-dd').format(_selectedDate);
      await _service.saveAttendance(
        dateStr, _selectedPeriod, _academicYearId!, _yearId!, _departmentId!, _sectionId, _students!
      );
      ...
      children: [
        Padding(
          ...
          child: Row(
            ...
            children: [
              TextButton(onPressed: () => _markAll('PRESENT'), child: const Text('Mark All Present')),
              TextButton(onPressed: () => _markAll('ABSENT'), child: const Text('Mark All Absent')),
            ],
          ),
        ),
        Expanded(
                ...
                child: ListTile(
                  ...
                  trailing: SegmentedButton<String>(
                    ...
                    onSelectionChanged: (Set<String> newSelection) {
                      setState(() {
                        _students![index] = s.copyWith(status: newSelection.first);
                      });
                    },
                  ),
                ),
